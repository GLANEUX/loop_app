...
import { getUserProfile, UserProfile, getInstrumentLevelLabel } from "@/lib/user";
...
export default function UserProfileContent({ profileId, onBack }: UserProfileContentProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [data, matches] = await Promise.all([
        getUserProfile(profileId, sessionToken),
  ...
  const avatarSource = useMemo(() => {
    if (profile?.hasAvatar) {
      ...
      return {
        uri: `${Env.API_URL}/user/profiles/${profileId}/avatar${cacheBreaker}`,
    ...
    <View style={styles.safeArea}>
      ...
      >
        <View style={styles.heroSection}>
          <Image source={avatarSource} style={styles.heroImage} contentFit="cover" cachePolicy="memory-disk" />
          <LinearGradient colors={["rgba(1,12,19,0.3)", "transparent", Palette.bgBlack]} style={StyleSheet.absoluteFill} />
          ...
          <View style={styles.heroInfo}>
            ...
            <View style={styles.locationRow}>
              ...
              <Text style={styles.locationText}>{profile?.distance ? `${profile.distance.toFixed(0)} km` : "Proche de toi"}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.contentBody}>
          {isMatched ? (
            <TouchableOpacity style={styles.primaryButton} activeOpacity={0.9} onPress={() => router.push(`/message/${matchId}`)}>
              <Text style={styles.primaryButtonText}>Envoyer un message</Text>
            </TouchableOpacity>
          ) : (
            <View style={styles.matchActionsRow}>
              <TouchableOpacity style={styles.secondaryButton} activeOpacity={0.8} onPress={handlePass}>
                <Text style={styles.secondaryButtonText}>PASSER</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.primaryButtonSmall} activeOpacity={0.8} onPress={handleLike}>
          ...
          <View style={styles.infoCard}>
            ...
            <Text style={styles.bioText} numberOfLines={isAboutExpanded ? undefined : 5}>
              {profile?.bio?.trim().length ? profile.bio : "Ce musicien n'a pas encore rédigé sa biographie."}
            </Text>
          ...
          {genres.length > 0 && (
            <View style={styles.infoCard}>
              ...
              <View style={styles.tagGrid}>
                {genres.map((label) => (
                  <View key={label} style={[styles.tag, styles.tagSecondary]}><Text style={[styles.tagText, styles.tagTextSecondary]}>{label}</Text></View>
                ))}
              </View>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  primaryButton: { backgroundColor: Palette.primary, paddingVertical: 18, borderRadius: 16, alignItems: "center", justifyContent: "center", marginBottom: 32, shadowColor: Palette.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 },
  primaryButtonText: { ...Typography.bodyBold, color: Palette.bgWhite, fontSize: 17 },
  matchActionsRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 16, marginBottom: 32, paddingTop: 8 },
  primaryButtonSmall: { flex: 1, backgroundColor: Palette.primary, paddingVertical: 18, borderRadius: 16, alignItems: "center", justifyContent: "center", shadowColor: Palette.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4 },
  secondaryButton: { flex: 1, backgroundColor: "#3B3F43", paddingVertical: 18, borderRadius: 16, alignItems: "center", justifyContent: "center" },
  secondaryButtonText: { ...Typography.bodyBold, color: Palette.bgWhite, fontSize: 17 },
